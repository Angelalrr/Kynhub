...
local _arEnabled, _arFrozen, _arDisabledRemotes, _arRemoteWatcher, _arSteppedConn, _arStateConn = false, false, {}, nil, nil, nil
local _arBlockedStates = {
    [Enum.HumanoidStateType.Ragdoll] = true,
    [Enum.HumanoidStateType.FallingDown] = true,
    [Enum.HumanoidStateType.Physics] = true,
    [Enum.HumanoidStateType.Dead] = true,
}
local function _arForceNormal(character)
    local hum = character:FindFirstChildOfClass("Humanoid"); local hrp = character:FindFirstChild("HumanoidRootPart")
    if not hum or not hrp then return end
    pcall(function() hum.Health = hum.MaxHealth end)
    pcall(function() hum:ChangeState(Enum.HumanoidStateType.RunningNoPhysics) end)
    if not _arFrozen then _arFrozen=true; hrp.Anchored=true; hrp.AssemblyLinearVelocity=Vector3.zero; hrp.AssemblyAngularVelocity=Vector3.zero; hrp.CFrame = hrp.CFrame + Vector3.new(0,1.5,0) end
end
local function _arRelease(character)
    local hrp = character:FindFirstChild("HumanoidRootPart")
    if hrp and _arFrozen then hrp.Anchored=false; _arFrozen=false end
end
local function _arRestoreMotors(character)
    for _, v in ipairs(character:GetDescendants()) do if v:IsA("Motor6D") then v.Enabled=true elseif v:IsA("Constraint") then v.Enabled=false end end
end
local function _arDisconnect()
    if _arSteppedConn then _arSteppedConn:Disconnect(); _arSteppedConn=nil end
    if _arStateConn then _arStateConn:Disconnect(); _arStateConn=nil end
end
local function _arInit(character)
    _arDisconnect()
    local hum = character:WaitForChild("Humanoid", 10) if not hum then return end
    if _arEnabled then for state in pairs(_arBlockedStates) do pcall(function() hum:SetStateEnabled(state, false) end) end end
    _arStateConn = hum.StateChanged:Connect(function(_, new) if _arEnabled and _arBlockedStates[new] then _arForceNormal(character); _arRestoreMotors(character) end end)
    _arSteppedConn = RunService.Stepped:Connect(function()
        if not _arEnabled then _arRelease(character); return end
        if _arBlockedStates[hum:GetState()] then _arForceNormal(character) else _arRelease(character) end
        if _arEnabled then pcall(function() hum.Health = hum.MaxHealth end) end
    end)
end
local function _arKillRemote(remote)
    if not remote:IsA("RemoteEvent") or _arDisabledRemotes[remote] then return end
    local name = remote.Name:lower()
    local keywords = {"useitem", "combatservice", "ragdoll"}
    for _, key in ipairs(keywords) do
        if name:find(key) then
            _arDisabledRemotes[remote] = {}
            pcall(function()
                for _, c in ipairs(getconnections(remote.OnClientEvent)) do
                    if c.Disable then c:Disable(); table.insert(_arDisabledRemotes[remote], c) end
                end
            end)
            break
        end
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if _arEnabled then task.wait(0.4); _arInit(char) end
end)
if LocalPlayer.Character then
    task.spawn(function() task.wait(0.5); if _arEnabled then _arInit(LocalPlayer.Character) end end)
end
...
_G.KYNAddToggle("Misc", {
    ...
    Callback = function(state)
        ...
        if state then
            ...
            if char then task.spawn(function() _arInit(char) end) end
            for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do pcall(function() _arKillRemote(obj) end) end
            if not _arRemoteWatcher then
                _arRemoteWatcher = ReplicatedStorage.DescendantAdded:Connect(function(obj) pcall(function() _arKillRemote(obj) end) end)
            end
        else
            _arDisconnect()
            for remote, conns in pairs(_arDisabledRemotes) do for _, c in ipairs(conns) do pcall(function() if c.Enable then c:Enable() end end) end end
            _arDisabledRemotes = {}
            if _arRemoteWatcher then _arRemoteWatcher:Disconnect(); _arRemoteWatcher = nil end
        end
    end
})
_G.KYNAddToggle("Misc", {
